...
//创世文件自定义内容
func genAppStateOptions(genesisCharter *GenesisCharter, ownerAddr crypto.Address, genesisContracts []contractInfo) GenesisAppState {
	...
	return GenesisAppState{Organization: "genesis", Token: tokenBasic, RewardStrategy: genesisRewards, Contracts: genesisContracts}
}
...
func genesis(chainID, password, pathOfCharter, pathOfOutput string) error {
	...
	genesisContracts, releaseVersion := findContract(filepath.Join(pathOfCharter, "smart-contract"))
	appState := genAppStateOptions(genesisCharter, ownerAcct.Addr, genesisContracts)
