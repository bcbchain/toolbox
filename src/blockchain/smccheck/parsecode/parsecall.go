	...
	for _, f := range fileMap {
		ast.Inspect(f, func(node ast.Node) bool {
			...
			case *ast.FuncDecl:
				...
				if exists, _ := v.isCalling(calleeList, fun); exists {
...
func (v *visitor) parseCallEx(calleeList []Callee, fileMap map[string]*ast.File) {
	for _, f := range fileMap {
		ast.Inspect(f, func(node ast.Node) bool {
			...
			case *ast.FuncDecl:
				fun := node.(*ast.FuncDecl)
				if exists, pos := v.isCalling(calleeList, fun); exists {
					newCallee := v.parseFunction(fun)
...
// nolint cyclomatic
func (v *visitor) isCalling(calleeList []Callee, d *ast.FuncDecl) (exists bool, pos token.Pos) {
...
func (this *CallNode) equals(n *CallNode) bool {
	if this.FuncName == n.FuncName {
		return true
	}
	return false
}
