...
// Create create receive type and value as string, then return variable that defined type and set value
func CreateForBvm(typeStr, valueStr string) (v interface{}, err error) {
	var initV interface{}
	// step 1
	initV, err = initVariableWithValue(typeStr, valueStr)
	if err != nil {
...
func initVariableWithValue(typeStr, valueStr string) (v interface{}, err error) {
	...
	if err != nil {
		switch temp.Type().String() {
		case "int64", "":
			err = errors.New("Please check if the parameter is int, int8, int16, int32, int64")
		case "uint64":
			err = errors.New("Please check if the parameter is uint, uint8, uint16, uint32, uint64")
		case "float64":
			err = errors.New("Please check if the parameter is float32, float64")
		case "string":
			err = errors.New("Please check if the parameter is string")
		case "bn.Number":
			err = errors.New("Please check if the parameter is bn.Number")
		case "bool":
			err = errors.New("Please check if the parameter is bool")
		case "uint8":
			err = errors.New("Please check if the parameter is byte")
		case "[]uint8":
			err = errors.New("Please check if the parameter is types.Hash, types.HexBytes, types.PubKey, []byte")
		}

		switch temp.Kind().String() {
		case "slice":
			err = errors.New(`Please check if the parameter is slice
          Examples of correct format: "[\"xxx..xxx\"]"`)
		case "map":
			err = errors.New(`Please check if the parameter is map
			Examples of correct format: "{\"key\":{\"key\":\"value\"}}"`)
		}
		return
	}
...
func createVariable(typeStr string, value interface{}, isMapKey bool) (v interface{}, err error) {
	...
	if v == nil {
		if strings.HasPrefix(typeStr, "map") {
			...
			//	v, err = createSliceVar(typeStr[2:], value)
			//}else if strings.HasSuffix(typeStr, "[]"){
			//	v, err = createSliceVar(typeStr[:len(typeStr)-2], value)
			v, err = createSliceVar(typeStr[2:], value)
		} else if strings.HasSuffix(typeStr, "[]") {
			v, err = createSliceVar(typeStr[:len(typeStr)-2], value)
		} else {
...
func createSliceVar(typeStr string, value interface{}) (v interface{}, err error) {
	...
	for i := 0; i < reflect.ValueOf(value).Len(); i++ {
		var v1 interface{}
		v1, err = createVariable(typeStr, reflect.ValueOf(value).Index(i).Interface(), false)
		if err != nil {
			return
		}
		temp = append(temp, v1)
...
func typeFromString(typeStr string) reflect.Type {
	...
	if strings.HasPrefix(typeStr, "map") {
		...
		sliceBaseTypeStr := typeStr[:len(typeStr)-2]
